...
- **Directly re-run for this ledger:** in this router worktree, `git diff --check HEAD` exited 0
  ...
  0.02s**) on 2026-07-12.
- **Actionlint:** it was not installed locally when this ledger was written.  The router's
  `make ci-local` prints a skip in that condition, while its workflow installs a pinned
  actionlint binary.  Therefore no row below claims a local actionlint pass or a remote workflow
  pass.
...
| GeneReviews R-01 | `36d9f73` + `a9c8f947cb421ee248a1fc45dcadb6eef23a5d8f` | `uv run pytest tests/unit/test_corpus_ingest_ceilings.py -q`; `make ci-local` |
| GeneReviews R-06 | `347a562` + `a9c8f947cb421ee248a1fc45dcadb6eef23a5d8f` | `uv run pytest tests/test_github_actions_pinning.py -q`; `make ci-local` |
...
| Orphanet R-06 | `f7626b1` + `fbf99c661808028cb463592201cc4d0e75f32582` | `uv run pytest tests/unit/test_docs_and_ci_contracts.py -q`; `make ci-local` |
| MGI R-07 | `99a00144214d3b6d1f90809032b7d6ddef88bcfb` | `uv run pytest tests/unit/test_cli.py tests/integration/test_live.py -q`; `make ci-local` |
...
deadlines in `a9c8f947cb421ee248a1fc45dcadb6eef23a5d8f`.
...
| Valid corpus ingestion remains green | Established GeneReviews `make ci-local` pass; remote CI remains pending. |
...
| --- | --- |
| GeneReviews #94: all non-local actions are full SHA pins; nested/reusable workflows are scanned | `347a562` (carried by `a9c8f947cb421ee248a1fc45dcadb6eef23a5d8f`); `tests/test_github_actions_pinning.py::test_external_github_actions_are_sha_pinned_with_version_comments` recursively scans `.github`; focused test and `make ci-local` passed. |
...
| GeneReviews #94: workflow YAML/actionlint and jobs pass | YAML source is covered by local test/gate; **actionlint and remote jobs have not been run for this ledger and remain pending.** |
| Orphanet #14: no mutable/short action refs, including composite action | `f7626b1` (carried by `fbf99c661808028cb463592201cc4d0e75f32582`); `tests/unit/test_docs_and_ci_contracts.py::test_github_action_pin_check_recurses_and_rejects_version_tags` and follow-up `test_github_action_pin_check_rejects_mutable_docker_actions`; focused test and `make ci-local` passed. |
...
| Orphanet #14: workflow YAML/actionlint and jobs pass | **Pending:** no local actionlint or remote workflow result is claimed. |

### R-07 — hermetic, parallel-safe tests

| Issue / acceptance criterion | Implementation and local evidence |
| --- | --- |
| MGI #16: refresh unit passes with network disabled | `99a00144214d3b6d1f90809032b7d6ddef88bcfb`; `tests/unit/test_cli.py::test_cli_refresh_builds` patches at the runtime lookup site under the unit network-deny fixture. |
| MGI #16: accidental unit-network access fails clearly | Same commit adds the autouse unit fixture in `tests/conftest.py`; the failure message is `network access is disabled in unit tests`. |
| MGI #16: live coverage is opt-in and bounded | `tests/integration/test_live.py::test_live_markers_download_has_an_explicit_timeout`; focused test and `make ci-local` passed. |
...
3. Run actionlint in the relevant workflow repositories (or rely on the pinned CI installation)
   and record its result.  Local actionlint is explicitly not evidenced here.
4. Before deployment, perform the operator-owned production rollout/re-pin checks.  This work
