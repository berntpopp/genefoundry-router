...
| Watcher-died detection | **Dead-man's-switch (healthchecks.io)** | A scheduled check can stop silently (GH drops runs under load; disables after 60 days). A heartbeat that the monitor *expects* turns "didn't run" into an alert. Free, open-source, self-hostable. |
...
Final step, `if: always()`: `curl -fsS -m 10 --retry 3 -o /dev/null "$HEALTHCHECKS_PING_URL"`. The
ping means "the job ran," independent of the drift result (drift is signalled by the issue). A
missing ping → healthchecks.io alerts. The check is configured for a 6h period + ~45 min grace
(covers GH cron delay up to ~30 min + run time). `HEALTHCHECKS_PING_URL` is a repo secret; the step
is skipped gracefully if unset (so forks/PRs without the secret don't error).
...
| `HEALTHCHECKS_PING_URL` | repo secret | dead-man's-switch ping URL; step no-ops if unset |
...
| Scheduled run dropped by GH / workflow disabled (60-day) | missing heartbeat | healthchecks.io alert |
| GH-wide Actions outage | missing heartbeat | healthchecks.io alert |
...
- The heartbeat ping carries no sensitive data ("a run happened"); healthchecks.io may be
  self-hosted if a third-party ping is undesirable (e.g. for the hospital deployment).
...
1. **Setup (one-time):** create a healthchecks.io check (period 6h, grace ~45 min), copy its ping
   URL into the `HEALTHCHECKS_PING_URL` repo secret; create the `tool-drift` label.
...
2. Self-host healthchecks.io from the start (data-residency) vs the free hosted tier? **Default:
   hosted free tier**; document self-hosting as a drop-in for the on-prem deployment.
